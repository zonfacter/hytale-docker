...
def apply_patches(dashboard_dir: Path):
    ...
    if old_get_service in content and "DOCKER_MODE" in content:
    ...
    if old_get_logs in content and "DOCKER_MODE" in content:
    ...
    if old_backup_func in content and "DOCKER_MODE" in content:
        ...
        if marker in content:
            ...
            print("WARNING: Could not find build_exec_start marker for backup frequency wrapper", file=sys.stderr)
    ...
    if old_backup_get in content and "DOCKER_MODE" in content:
    ...
    if old_backup_set in content and "DOCKER_MODE" in content:
